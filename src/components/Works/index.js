...
const Works = () => {
    function displayWorkDetails(e) {
        ...
        let globalContainer = e.target.querySelector('.global-container')
            ...
            <div className={'w-2/3 mx-auto grid grid-cols-3 place-items-center place-items-stretch items-stretch gap-4 mt-10'}>
                {
                    Object.values(works).map((work, index) => {
                      return(
                          <div className={'overflow-y-hidden overflow-x-hidden relative h-52 w-full bg-cover bg-center'} style={{backgroundImage: `url(${work.project_image})`}} key={index} onMouseEnter={displayWorkDetails} onMouseLeave={hideWorkDetails}>
                              <div className={'absolute z-10 h-full w-full bg-black/80 flex flex-col place-content-center hidden global-container'}>
